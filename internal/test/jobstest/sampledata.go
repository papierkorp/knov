...
func resetAndSeed() error {
	...
	if err := test.SeedMetadataNoRefresh(&files.Metadata{Path: "media/" + mediaPath(usedMediaFile)}); err != nil {
		return err
	}
	...
	if err := test.SeedMetadataNoRefresh(&files.Metadata{Path: "media/" + mediaPath(orphanMediaFile)}); err != nil {
		return err
	}
