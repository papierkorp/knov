...
func HandleSetRepositoryURL(w http.ResponseWriter, r *http.Request) {
	...
	if repositoryURL == "" {
		logging.LogWarning("empty repositoryUrl")
	...
	if err != nil {
		...
		logging.LogInfo("trying to add remote instead...")
		...
		if err != nil {
			logging.LogError("add remote failed with error: %v, output: %s", err, string(output))
